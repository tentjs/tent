...
function e(as, nodes = [], opts = {}) {
  ...
  const $context = new Proxy(context, {
    ...
    set(target, key, value) {
      if (key.startsWith('$') && key !== '$props') {
        throw new Error(`Cannot set reserved property name: ${key}.`)
      }
    ...
    function traverse(children, compare) {
      ...
      for (const c of children) {
        ...
        if (dom && !dom.isEqualNode(child)) {
          ...
          if (dom && dom.attributes.key !== child.attributes.key) {
            dom.replaceWith(child)
            i++
            continue
          }
