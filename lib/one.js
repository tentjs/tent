...
class One {
  ...
  render (target = this.el, state = this.state) {
  ...
  on (el, events) {
    ...
    events.forEach(event => {
      ...
      // TODO: Not sure this is necessary when the event is overwritten
      if (el[`on${e}`]) {
        el[`on${e}`] = null
      }

      el[`on${e}`] = this.methods[key].bind(this)
    })
  }
  ...
  mount () {
    ...
    if (config.el) {
      if (!(config.el instanceof window.HTMLElement)) {
        throw new Error('config.el should be an HTMLElement')
      }
