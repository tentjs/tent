...
function template (config) {
  ...
  if (tmpl.content.children.length > 1) {
    throw new Error('One->template: You can only have one root element in your template.')
  }

  const clone = tmpl.content.children[0].cloneNode(true)
  const cmpt = components[config.name].find(c => c.uid === config.uid)
