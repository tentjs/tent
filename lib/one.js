...
/**
 * @idea
 * put `o-for="path.to.arr"` on the parent
 * save the `el`, `arr` and `fn` on parent.$one.loop
 *
 * When running `render()` we should check for `o-for`,
 * and then run the `loop()` with the parent.$one.loop.
 *
 * Compare `parent.$one.loop.arr` with `config.state[path.to.arr]`
 *  - if there are less items in the new arr, remove overflowing elements
 *  - if there are more items in the new arr, append new elements
 *
 * Compare the new DOM with the one currently rendered.
 */
function loop (el, arr, key, fn, p = null) {
  ...
  parent.$one = {
    ...
    store: this.store,
    state: this.state,
    // TODO: If the user assigns the `arr` via state as a new array,
    // i.e. state.items = [...state.items] the reference to parent.$one.loop.arr,
    // is broken. Find a way to keep `arr` synced no matter how the user assigns state.
    loop: { parent, el, key, arr, fn, store: this.store }
  }

  for (const child of parent.children) {
    const k = Number(child.getAttribute('o-key'))
    if (!arr.find(x => x[key] === k)) {
      child.remove()
    }
  }

  parent.setAttribute('o-for', '')

  arr.forEach((item, index) => {
    const clone = el.cloneNode(true)

    if (!item[key]) {
      throw new Error(`One->loop: The key "${key}" does not exist on ${JSON.stringify(item)}`)
    }
    ...
    clone.setAttribute('o-key', item[key])

    if (parent.children[index]) {
      render(parent.children[index].$one)
    } else {
      // TODO: Be more clever about how to append the element.
      // By appending it to the parent, it ruins the order of the elements.
      // Or, require that looped elements don't have any siblings on first render.
      parent.append(clone)
    }
...
function traverse (el, config) {
  Array.from(el.attributes).forEach(attr => {
    ...
    if (attr.name === 'o-for') {
      const { el: elm, key, arr, fn, parent } = el.$one.loop
      loop(elm, arr, key, fn, parent)
    }
  })
