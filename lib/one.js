...
function transferEvents (from, to) {
  for (const prop in from) {
    if (prop.startsWith('on') && typeof from[prop] === 'function') {
      to[prop] = from[prop]
    }
  }
  ...
  if (from.children.length) {
    Array.from(from.children).forEach((child, i) => {
      transferEvents(child, to.children[i])
    })
  }
}
