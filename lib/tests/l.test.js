...
test('that you cannot use data if there is no data object', () => {
  expect(() => {
    L('div', ({data}) => `Hello ${data.foo}`)
...
test('element with props', () => {
  ...
  const Test = L(
    'div',
    ({data}) => (data.name ? `Hello ${data.name}` : 'Hello'),
    {
      props: ['name'],
      data: {},
    }
...
test('that it is required to setup `props: []` to get access to props', () => {
  ...
  expect(screen.queryByText('Hello')).toBeTruthy()
  fireEvent.click(screen.queryByText('Click me!'))
  expect(screen.queryByText('Hello Joe')).toBeNull()
})
