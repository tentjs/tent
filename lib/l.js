import {createUUID, replaceWith, remove, getStyles, getChildren} from './utils'
...
function L(as, nodes = [], opts = {}) {
  ...
  if (props && Array.isArray(props)) {
    if ($data == null) {
      throw new Error('You must provide a data object to use props')
    }
    for (const prop of props) {
      Object.defineProperty(el, prop, {
        set: function (newValue) {
          if ($data[prop] !== newValue) {
            $data[prop] = newValue
          }
        },
      })
  ...
  function render() {
    ...
    function t(children, compare) {
      ...
      for (const child of children) {
        ...
        if (
          (dom instanceof Node && !(child instanceof Node)) ||
          (!(dom instanceof Node) && child instanceof Node)
        ) {
          throw new Error(
