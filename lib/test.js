function L(as = 'div', props) {
  const {children, root, data, ...attributes} = props;
  if (as instanceof HTMLElement) {
    return as;
  }
  const div = document.createElement(as ? as : 'div');
  for (const attr in attributes) {
    const a = attributes[attr];
    if (typeof a === 'function') {
      div[attr] = data != null ? a.bind(data) : a;
    } else {
      div.setAttribute(attr, props[attr]);
    }
  }
  const $data = data ? new Proxy(data, {
    get(target, key) {
      return target[key];
    },
    set(target, key, value) {
      const s = Reflect.set(target, key, value);
      render();
      return s;
    },
  }) : null;
  const render = () => {
    const ch = typeof children === 'function' ? children($data) : Array.from(children);
    if (div.children.length === 0) {
      div.append(...ch);
      return;
    }
    const t = (ch, compare) => {
      let i = 0;
      for (const child of ch) {
        const dom = compare.children[i];
        if (dom && !dom.isEqualNode(child)) {
          if (dom.children.length === 0) {
            dom.replaceWith(child);
          }
          const domChildren = Array.from(dom.children);
          const childChildren = Array.from(child.children);
          const childWasAdded = childChildren.length > domChildren.length;
          const childWasRemoved = domChildren.length > childChildren.length;
          if (childWasAdded) {
            let j = 0;
            for (const c of childChildren) {
              if (!domChildren[j]) {
                dom.append(c);
              }
              j++;
            }
          }
          if (childWasRemoved) {
            let j = 0;
            for (const c of domChildren) {
              if (!childChildren[j]) {
                for (const attr of c.attributes) {
                  if (attr.name.startsWith('on')) {
                    c[attr.name] = null;
                  }
                }
                c.remove();
              }
              j++;
            }
          }
          t(childChildren, dom);
        }
        i++;
      }
    };
    t(ch, div);
  };
  render();
  if (root) {
    document.body.append(div);
  } else {
    return div;
  }
}

const Custom = L('div', {
  data: {
    amount: 0,
  },
  children: (data) => ([
    L('h1', {children: ['Hello world']}),
    L('p', {children: ['This is a paragraph']}),
    L('p', {children: [`The button was clicked ${data.amount} times.`]}),
    L('button', {
      name: 'my-button',
      children: [
        'Click me'
      ],
      onclick() {
        data.amount = data.amount + 1;
      },
    }),
  ]),
});

L('div', {
  root: true,
  children: [
    L('div', {
      data: {
        amount: 0,
        items: [
          {id: 1, name: 'JS', description: 'JavaScript is nice', subtitle: 'JSX'},
          {id: 2, name: 'Svelte', description: 'Svelte is cool', subtitle: 'SvelteX'},
          {id: 3, name: 'Praxy', description: 'Praxy is awesome', subtitle: 'PraxyX'},
        ],
      },
      children: (data) => [
        L('ul', {
          children: data.items.map((item) =>
            L('li', {
              children: [
                L('h2', {children: [item.name]}),
                L('p', {children: [item.description]}),
                L('p', {children: [
                  L('span', {children: [item.subtitle]})
                ]}),
              ],
            })
          ),
        }),
        L(Custom, {}),
        L('button', {
          name: 'my-button',
          children: [
            data.amount ? `Clicked ${data.amount} times` : 'Click me'
          ],
          onclick() {
            data.amount = data.amount + 1;
            const items = [...data.items];
            items[0].name = 'React';
            items[1].subtitle = 'ReactX';
            data.items = items;
          },
        }),
      ],
    }),
  ],
});
