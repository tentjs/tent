...
function render(el, nodes, $context) {
  ...
  if (el.attributes.length) {
    for (const attr of el.attributes) {
      if (el.$else.attrs?.[attr.name]) {
        el.setAttribute(
          attr.name,
          el.$else.attrs[attr.name]({context: $context})
        )
      }
    }
  }
...
function diff(children, compare, $context) {
  ...
  for (const c of children) {
    ...
    if (!dom || !child || dom.isEqualNode(child)) {
      i++
      continue
    }
    ...
    if (child.attributes.length) {
      for (const attr of child.attributes) {
        ...
        if (child.$else.attrs?.[attr.name]) {
          child.setAttribute(
            attr.name,
            child.$else.attrs[attr.name]({context: $context})
          )
        }
      }
    }
