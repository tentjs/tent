class Praxy {
  ...
  component(cmpt, mounted) {
    ...
    if (tmp.content.children.length > 1) {
      ...
      if (tmp.content.children[0].hasAttributes()) {
  ...
  #map(node, uuids, data, map) {
    ...
    for (let i = 0; i < node.children.length; i++) {
      ...
      if (child.hasAttributes() && child.hasAttribute('px-for')) {
        return;
      }

      if (
        Array.from(child.childNodes)?.some((c) => c.nodeValue?.match(/{{(.*?)}}/g)) ||
        (child.hasAttributes() && child.hasAttribute('k'))
      ) {
        ...
        const isFor =
          (parent.hasAttributes() && parent.hasAttribute('px-for')) ||
  ...
  #render(root, map) {
    Object.keys(map).forEach((key) => {
      const m = map[key];
      const domEl =
        root.children.length === 0 || (root.hasAttributes() && root.getAttribute('k') === key)
          ? root
  ...
  #closest(el, attrName, attrValue, end = document.body) {
    ...
    while (parentNode != null) {
      const stop =
        typeof end === 'string' ? parentNode.hasAttributes() && parentNode.hasAttribute(end) : end;
      if (parentNode === stop) {
        return;
      }
      if (attrValue == null) {
        if (parentNode.hasAttributes() && parentNode.hasAttribute(attrName)) {
          return parentNode;
        }
      } else if (parentNode.hasAttributes() && parentNode.getAttribute(attrName) === attrValue) {
        return parentNode;
      }
