class Praxy {
  ...
  async component(cmpt, mounted, unmounted) {
    ...
    if (!customEl && !target) {
      throw new Error(
        `Praxy->component: You must provide a target for "${cmpt.name}" or use a custom element.`
      );
    }
    ...
    if (el.children.length) {
      ...
      throw new Error(
        `Praxy->component: You must provide a template for "${cmpt.name}" or use a custom element.`
      );
    }
