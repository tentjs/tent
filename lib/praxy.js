class Praxy {
  ...
  async component(name, options, mounted, unmounted) {
    ...
    if (!name) {
      throw new Error(
        `Praxy->component: You must provide a name for your component.`
      )
    }
    if (this.#components[name]) {
      throw new Error(`Praxy->component: "${name}" already exists`)
    }
    ...
    if (!customEl && !target) {
      throw new Error(
        `Praxy->component: You must provide a target for "${name}" or use a custom element.`
      )
    }
    const el = customEl ? customEl : document.querySelector(target)

    if (!el) {
      throw new Error(`Praxy->component: Your mount point doesn't exist`)
    }
    ...
    if (el.children.length) {
      ...
      tmp.innerHTML = options.template.trim()
    } else {
      throw new Error(
        `Praxy->component: You must provide a template for "${name}" or use a custom element.`
      )
    }
    if (tmp.content.children.length > 1 || tmp.content.children.length === 0) {
      customEl?.remove()
      throw new Error(
        `Praxy->component: Your template for "${name}" must have a single root element.`
      )
    }

    const root = options.template
      ? tmp.content.children[0].cloneNode()
      : customEl.children[0].cloneNode()

    let initialData = {}
    const sample =
      options.inherit && this.#components[options.inherit]?.data
        ? this.#components[options.inherit].data
        : options.data
    if (typeof sample === 'function') {
      if (customEl) {
        while (customEl.firstChild) {
          customEl.removeChild(customEl.lastChild)
        }
      }
    ...
    if (options.store?.init && typeof options.store.init === 'function') {
      ...
      if (typeof o !== 'object') {
        throw new Error(
          `Praxy->component: Your store for "${name}" must return an object.`
        )
      }
      ...
      for (const k in o) {
        if (o.hasOwnProperty(k)) {
          this.#store[k] = store?.[k] ? store[k] : o[k]
        }
      }
    }
  ...
  #map(node, uuids, data, map) {
    ...
    for (let i = 0; i < node.children.length; i++) {
      ...
      const isFor =
        (child.parentNode.attributes &&
          child.parentNode.hasAttribute('px-for')) ||
      ...
      if (child.attributes) {
        ...
        for (const attr of clone.attributes) {
          ...
          if (attr.name === 'px-if') {
            ...
            if (arr.length === 0) {
              throw new Error(
                `Praxy->map: "${attr.value}" is not a valid array.`
              )
            }
            map[uuid] = {
              clone,
            ...
            for (let i = 0; i < arr.length; i++) {
              ...
              if (isTernary) {
                ...
                if (!val.includes("'") && !val.includes('"')) {
                  ...
                  if (loopItem?.[val] || rootValue) {
                    map[uuid].classes.push(loopItem[val] ?? rootValue)
                  }
                } else {
                  map[uuid].classes.push(val.replaceAll(/['"]/g, ''))
                }
              } else {
                const v = this.#getValue(child, data, [cl], cl, isFor)
                if (cl.includes("'") || cl.includes('"')) {
                  map[uuid].classes.push(cl.replaceAll(/['"]/g, ''))
                } else if (v) {
                  map[uuid].classes.push(v)
                }
              }
            }
  ...
  #render(root, map) {
    Object.keys(map).forEach((key) => {
      ...
      const domEl =
        ...
        (root.attributes && root.getAttribute('k') === key)
          ? root
      ...
      if (
        domEl &&
        m.classes?.length &&
        m.classes.join(' ') !== domEl.className
      ) {
        ...
        if (index == null) {
          ...
          this.#events.forEach(({event, target, fire}) => {
            if (!e.includes(target)) {
              this.#on(event, target, fire, {parent: c, silent: true})
            }
            e.push(target)
          })
          c.setAttribute('i', i)
  ...
  #getValue(child, data, k, match, isFor) {
    ...
    if (isFor) {
      const index =
        this.#closest(child, 'i')?.getAttribute('i') ?? child.getAttribute('i')
  ...
  #closest(el, attrName, attrValue, end = document.body) {
    ...
    while (parent != null) {
      const stop =
        typeof end === 'string'
          ? parent.attributes && parent.hasAttribute(end)
          : end
      if (parent === stop) {
        return
      }
      if (attrValue == null) {
        ...
        parent.attributes &&
        parent.getAttribute(attrName) === attrValue
      ) {
        return parent
      }
  ...
  #on(
    ...
    const {parent, silent, triggers, preventDefault, ...nativeOptions} = options
    ...
    for (const el of els) {
      this.#events.push({event, target, fire, el})

      el.addEventListener(event, handler.bind(this), nativeOptions)

      if (triggers) {
        el.addEventListener('keypress', triggerHandler)
      }
    }
  }
      ...
      },
      {
        set: (data, key, value) => {
          ...
          for (const [, cmpt] of components) {
            if (
              cmpt.store?.subscribe?.includes(key) &&
    ...
    new MutationObserver((mutations) => {
      const run = (node, ev) => {
        ...
        if (node.children?.length) {
          Array.from(node.children).forEach((child) => run(child, ev))
        }
      }
      for (let i = 0; i < mutations.length; i++) {
        ...
        if (mutation.removedNodes) {
          ...
          for (const ev of this.#events) {
            removedNodes.forEach((node) => run(node, ev))
          }
          const removed = removedNodes.map((x) => x.className).filter(Boolean)
          for (const name in this.#components) {
            if (removed.find((x) => x.includes(name))) {
              this.#components[name].unmounted?.()
            }
          }
        }
      }
