class Praxy {
          ...
          for (const [, cmpt] of components) {
            if (cmpt.store?.subscribe?.includes(key) && cmpt.data[key] !== data[key]) {
              ...
              if (ctx?.store?.persist) {
                ...
                if (!storage) {
                  throw new Error(
                    `Praxy->store: "${ctx.store.persist}" is not a valid storage type.`
                  );
                }
  ...
  async component(cmpt, mounted) {
    ...
    if (cmpt.store?.init && typeof cmpt.store.init === 'function') {
      ...
      if (typeof o !== 'object') {
        throw new Error(`Praxy->component: Your store for "${cmptName}" must return an object.`);
      }
      for (const k in o) {
        if (o.hasOwnProperty(k)) {
          // check if the key is already in sessionStorage/localStorage if persist is set
