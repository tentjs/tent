class Praxy {
  ...
  async component(name, options, mounted, unmounted) {
    ...
    if (options.store?.init && typeof options.store.init === 'function') {
      ...
      if (typeof o !== 'object') {
        throw new Error(`Praxy->component: Your store for "${name}" must return an object.`);
      }
      ...
      if (child.attributes) {
        ...
        for (const attr of clone.attributes) {
          ...
          if (attr.name === 'px-if') {
            ...
            if (arr.length === 0) {
              throw new Error(`Praxy->map: "${attr.value}" is not a valid array.`);
            }
            map[uuid] = {
              clone,
              classes: [],
  ...
  #render(root, map) {
    Object.keys(map).forEach((key) => {
      ...
      if (domEl && m.classes?.length && m.classes.join(' ') !== domEl.className) {
        ...
        set: (data, key, value) => {
          ...
          for (const [, cmpt] of components) {
            ...
            if (ctx?.store?.persist) {
              ...
              if (!storage) {
                throw new Error(`Praxy->store: "${data.$persist}" is not a valid storage type.`);
              }
