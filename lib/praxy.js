class Praxy {
  ...
  component(cmpt, mounted) {
    ...
    if (el == null) {
      throw new Error(
        `Praxy->component: Your mount point "${
  ...
  render(root, map) {
    Object.keys(map).forEach((key) => {
      ...
      const domEl =
        ...
        (root.hasAttributes() && root.getAttribute('k') === key)
          ? root
        ...
        const live = Array.from(nodes).map((node) => {
          ...
          if (node.nodeName === '#text') {
            n.nodeValue = n.nodeValue.replaceAll(/{{(.*?)}}/g, (match) => {
              ...
              if (isFor) {
                const index =
                  this.closest(child, 'i')?.getAttribute('i') ??
  ...
  closest(el, attrName, attrValue, end = document.body) {
    ...
    while (parentNode != null) {
      const stop =
        typeof end === 'string'
          ? parentNode.attributes && parentNode.hasAttribute(end)
          : end;
      if (parentNode === stop) {
        return;
      }
