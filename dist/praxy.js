...
class Praxy {
    ...
    #for(root, uuids, data, map, fors) {
        ...
        root.querySelectorAll("[px-for]")?.forEach((el)=>{
            ...
            if (firstRender) {
                ...
                Array.from(parent.children).find((c)=>!c.hasAttribute("i"))?.remove();
                return;
            }
            // sync nodes
            children.forEach((c)=>{
    ...
    #on(event, target, fire, options = {
        ...
        function triggerHandler(ev) {
            for(const trigger in triggers)if (ev.key.toLowerCase() === trigger.toLowerCase()) triggers[trigger](ev);
        }
        for (const el of els){
            this.#events.push({
                event,
            ...
            if (triggers) el.addEventListener("keypress", triggerHandler);
        }
    }
    #createStore(ctx) {
        if (!ctx.store) return {};
